...
func DrainEscrow(this js.Value, args []js.Value) interface{} {
	...
	handler := js.FuncOf(func(this js.Value, args []js.Value) interface{} {
		...
		go func() {
			...
			flipTxJson, err := drainEscrow(holder, uint64(amount))
			if err != nil {
...
func drainEscrow(holder solana.PublicKey, amount uint64) ([]byte, error) {
	instructions := make([]solana.Instruction, 0)
	txJson := []byte("{}")

	if drainIx := ops.DrainEscrow(holder, amount); drainIx != nil {
		instructions = append(
			instructions,
			drainIx,
		)
